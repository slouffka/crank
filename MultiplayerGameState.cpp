...
bool MultiplayerGameState::update(sf::Time dt)
{
    ...
    if (mConnected)
    {
        ...
        // Remove players whose ships were destroyed
        bool foundLocalShip = false;
        for (auto itr = mPlayers.begin(); itr != mPlayers.end(); )
        {
            // Check if there are no more local ships for remote clients
        ...
        else
        {
            ...
            if (mTimeSinceLastPacket > mClientTimeout)
            {
                ...
                mFailedConnectionText.setString("Lost commection to server");
                centerOrigin(mFailedConnectionText);
...
void MultiplayerGameState::handlePacket(sf::Int32 packetType, sf::Packet& packet)
{
    switch (packetType)
    {
        ...
        // Sent by the server to order to spawn player 1 ship on connect
        ...
        case Server::InitialState:
        {
            ...
            for (sf::Int32 i = 0; i < shipCount; ++i)
            {
                ...
                sf::Vector2f shipPosition;
                packet >> shipIdentifier >> shipPosition.x >> shipPosition.y >> hitpoints >> missileAmmo;

                Ship* ship = mWorld.addShip(shipIdentifier);
                ship->setPosition(shipPosition);
                ship->setHitpoints(hitpoints);
                ship->setMissileAmmo(missileAmmo);
        ...
        case Server::SpawnEnemy:
        {
            ...
            mWorld.addEnemy(static_cast<Ship::Type>(type), relativeX, height);
            mWorld.sortEnemies();
        } break;
        ...
        case Server::UpdateClientState:
        {
            ...
            for (sf::Int32 i = 0; i < shipCount; ++i)
            {
                ...
                bool isLocalShip = std::find(mLocalPlayerIdentifiers.begin(), mLocalPlayerIdentifiers.end(), shipIdentifier) != mLocalPlayerIdentifiers.end();
                if (ship && !isLocalShip)
                {
